...
def evaluate_role(client, company, role, location):
    ...
    for attempt in range(3):
        try:
            ...
            # Extract just the JSON object in case model adds preamble/postamble
            json_match = re.search(r'\{.*\}', raw, re.DOTALL)
            if json_match:
                raw = json_match.group(0)
            return extract_fields(raw)
        except json.JSONDecodeError:
            print(f"  ⚠️  JSON parse failed on attempt {attempt+1} for {company} — {role}")
            print(f"       Raw response: {raw[:200]}")
            time.sleep(1)
        except Exception as e:
            err_str = str(e)
            print(f"  ⚠️  API error on attempt {attempt+1}: {e}")
            # Parse Groq's "try again in Xm Y.Zs" and wait exactly that long
            wait_match = re.search(r'try again in (\d+)m([\d.]+)s', err_str)
            if wait_match:
                wait_secs = int(wait_match.group(1)) * 60 + float(wait_match.group(2)) + 2
                print(f"  ⏳ Daily token limit hit — waiting {wait_secs:.0f}s then retrying...")
...
def main():
    ...
    if not api_key:
        print("❌ No GROQ_API_KEY found. Create a .env file with: GROQ_API_KEY=your_key_here")
        return
    ...
    print(f"🤖 Scoring with Groq (llama-3.1-8b-instant)...")
